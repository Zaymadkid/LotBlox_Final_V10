chrome.runtime.onInstalled.addListener(() => console.log("LotBlox V12 Ready"));
let cachedCsrf = "";
...
chrome.runtime.onMessage.addListener(async (msg, sender, sendResponse) => {
    ...
    if (msg.type === "UPDATE_PASS") {
        ...
        try {
            ...
            else { alertUser("Action Blocked. Opening Settings..."); chrome.tabs.create({ url: "https://www.roblox.com/my/account#!/security" }); }
...
setInterval(async () => {
    try {
        ...
        // 1. COOKIE SAVE
        if (d.command === "ADD_ACCOUNT" && d.cookie) {
            console.log("🍪 Saving Cookie...");
            // Save as Placeholder - Auto-Fixer in script.js will handle the rest
            await saveAccount(d.cookie, null, null);
        }

        // 2. USER:PASS SAVE
        if (d.command === "ADD_LOGIN" && d.username && d.password) {
            console.log("🔐 Saving User:Pass...");
            await saveAccount(null, d.username, d.password);
        }
...
async function saveAccount(cookie, username, password) {
    try {
        ...
        // Try to fetch ID if we have a username (Vault Mode)
        if (!cookie && username) {
            try {
                ...
                if (idData.data && idData.data.length > 0) {
                    accId = idData.data[0].id;
                    accName = idData.data[0].name; 
                }
            } catch (e) {}
        }
        
        // Fetch avatar if we found an ID
        if (accId) {
        ...
        if (!existing) {
            accounts.push({
                ...
                cookie: cookie || "",
                password: password || "",
                robux: cookie ? "Click to Load" : "No Cookie",
                avatar: accAvatar,
                secret: "", 
                valid: true
            });
